    ...
    return (
        <>
            {cartOpen && showOverlay && (
                <div
                    className={`absolute inset-0 z-20 bg-black opacity-20 transition-opacity duration-150 `}
                    onClick={() => setCartOpen(false)}
                ></div>
            )}
            <section
                className={`fixed bottom-0 right-0 top-0 z-20 w-[480px] overflow-auto bg-white p-5 shadow-xl transition-all duration-500 2xs:w-full 2xs:p-3 3xs:p-2 ${
